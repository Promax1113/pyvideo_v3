...
def setup() -> dict:
    ...
    while runnning:
        ...
        item, act = choice.Menu(sections, ["edit", "reset"], ["save", "discard"], title="Choose an entry and what to do with it or continue.").ask()
        match act:
            case "edit":
                selected_section = item
                keys = []
                for _key, value in data.items(item):
                    keys.append(_key)

                key_choice = choice.Menu(keys,["edit"], ["back"], title=f"Choose a key to edit in the '{item}' section.").ask()
                if isinstance(key_choice, tuple) and key_choice[1] == "back":
                    continue
                new_value = choice.Input(f"New value? Current for [{item}][{key_choice}]: {data[item][key_choice]}  **Leave blank to keep current").ask()
                data[item][key_choice] = new_value
                

            case "save":
                if choice.Binary("Are you sure you want to save?").ask():     
                    with open(f"{os.getcwd()}/setup/config.ini", 'w') as configfile:
                        if data:
    ...
    return dict(data.items(selected_section))
